...
function updateSanityScore(node)
    ...
    if rActor then
        ...
        tMentalScores['cha'] = DB.getValue(nodeChar, 'abilities.charisma.score',
                                           0)
        tMentalScores['wis'] =
            DB.getValue(nodeChar, 'abilities.wisdom.score', 0)
        tMentalScores['int'] = DB.getValue(nodeChar,
                                           'abilities.intelligence.score', 0)

        tMentalScores['cha_effect'] = EffectManager35EDS.getEffectsBonus(rActor,
                                                                         'CHA',
                                                                         true)
        tMentalScores['wis_effect'] = EffectManager35EDS.getEffectsBonus(rActor,
                                                                         'WIS',
                                                                         true)
        tMentalScores['int_effect'] = EffectManager35EDS.getEffectsBonus(rActor,
                                                                         'INT',
                                                                         true)

        tMentalScores['cha_dmg'] = -1 *
                                       DB.getValue(nodeChar,
                                                   'abilities.charisma.damage',
                                                   0)
        tMentalScores['wis_dmg'] = -1 *
                                       DB.getValue(nodeChar,
                                                   'abilities.wisdom.damage', 0)
        tMentalScores['int_dmg'] = -1 *
                                       DB.getValue(nodeChar,
                                                   'abilities.intelligence.damage',
                                                   0)
        ...
        local tThreshold = {}
        tThreshold['cha'] = tMentalScores.cha + tMentalScores.cha_effect +
                                tMentalScores.cha_dmg
        tThreshold['wis'] = tMentalScores.wis + tMentalScores.wis_effect +
                                tMentalScores.wis_dmg
        tThreshold['int'] = tMentalScores.int + tMentalScores.int_effect +
                                tMentalScores.int_dmg

        local nHighestMentalStat = math.max(tThreshold.cha, tThreshold.wis,
                                            tThreshold.int)
        ...
        DB.setValue(nodeChar, 'sanity.threshold', 'number',
                    nSanityThreshold - nSanityThreshold % 1)
    end
end
...
function onInit()
    if Session.IsHost then
        DB.addHandler(DB.getPath('charsheet.*.abilities.charisma'),
                      'onChildUpdate', updateSanityScore)
        DB.addHandler(DB.getPath('charsheet.*.abilities.wisdom'),
                      'onChildUpdate', updateSanityScore)
        DB.addHandler(DB.getPath('charsheet.*.abilities.intelligence'),
                      'onChildUpdate', updateSanityScore)
        ...
        DB.addHandler(DB.getPath(CombatManager.CT_COMBATANT_PATH .. '.effects'),
                      'onChildDeleted', updateSanityScore)
    end
end
